...
func DecodeWIF(wif string) (*WIF, error) {
	...
	case dcrec.ECTypeSecp256k1:
		...
		ecType = dcrec.ECTypeSecp256k1
	case dcrec.ECTypeEdwards:
		privKeyBytes := decoded[3 : 3+32]
		curve := new(edwards.TwistedEdwardsCurve)
		curve.InitParam25519()
		privKey, _, _ = edwards.PrivKeyFromScalar(curve, privKeyBytes)
		ecType = dcrec.ECTypeEdwards
	case dcrec.ECTypeSecSchnorr:
		privKeyBytes := decoded[3 : 3+secp256k1.PrivKeyBytesLen]
		privKey, _ = secp256k1.PrivKeyFromScalar(privKeyBytes)
		ecType = dcrec.ECTypeSecSchnorr
	}
...
func (w *WIF) SerializePubKey() []byte {
	...
	case dcrec.ECTypeEdwards:
		pk = edwards.NewPublicKey(new(edwards.TwistedEdwardsCurve), pkx, pky)
	case dcrec.ECTypeSecSchnorr:
		pk = secp256k1.NewPublicKey(pkx, pky)
	}
