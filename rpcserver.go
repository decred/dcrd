...
func createTxRawResult(chainParams *chaincfg.Params, mtx *wire.MsgTx,
	...
	blkHeight int32, blkIdx uint32, chainHeight int32) (*dcrjson.TxRawResult, error) {
...
func handleGetBlock(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	blockReply := dcrjson.GetBlockVerboseResult{
		...
		Confirmations: uint32(1 + maxIdx - idx),
		Height:        int32(idx),
		Size:          int32(len(buf)),
	...
	if c.VerboseTx == nil || !*c.VerboseTx {
		...
		for i, tx := range txns {
			rawTxn, err := createTxRawResult(s.server.chainParams,
				...
				sha.String(), idx, uint32(i), maxIdx)
			if err != nil {
				return nil, err
			}
			rawTxns[i] = *rawTxn
...
func handleGetBlockHeader(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	if c.Verbose == nil || *c.Verbose {
		...
		blockHeaderReply := dcrjson.GetBlockHeaderVerboseResult{
			Hash:          c.Hash,
			Confirmations: uint32(1 + maxIdx - blk.Height()),
...
func handleGetCoinSupply(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	for i := 0; int32(i) < tipHeight+1; i++ {
		...
		blockSha, err := s.server.db.FetchBlockShaByHeight(int32(i))
		if err != nil {
		...
		if int32(i) < params.StakeValidationHeight {
			supply += (work + tax)
		} else {
...
func handleGetRawTransaction(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	rawTxn, err := createTxRawResult(s.server.chainParams, mtx,
		txHash.String(), blkHeader, blkHashStr, blkHeight, blkIndex, chainHeight)
	if err != nil {
		return nil, err
	}
	return *rawTxn, nil
...
func pruneOldBlockTemplates(s *rpcServer, bestHeight int32) {
	pool := s.templatePool
	for rootHash, blkData := range pool {
		height := int32(blkData.msgBlock.Header.Height)
...
func handleSearchRawTransactions(s *rpcServer, cmd interface{},
	...
	for i, txReply := range addressTxs {
		...
		rawTxn, err := createTxRawResult(s.server.chainParams, mtx,
			txHash, blkHeader, blkHashStr, blkHeight, blkIndex, maxIdx)
