	...
	if c.VerboseTx == nil || !*c.VerboseTx {
		...
		for i, tx := range txns {
			rawTxn, err := createTxRawResult(s.server.chainParams,
				...
				blockHeader, sha.String(), idx, maxIdx)
			if err != nil {
				return nil, err
			}
			rawTxns[i] = *rawTxn
...
func handleSearchRawTransactions(s *rpcServer, cmd interface{},
	...
	rawTxns := make([]dcrjson.SearchRawTransactionsResult, len(addressTxs), len(addressTxs))
	for i, txReply := range addressTxs {
...
func handleTicketVWAP(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	totalValue := int64(0)
	for i := start; i <= end; i++ {
		blockHash, err := s.server.db.FetchBlockShaByHeight(int64(i))
		if err != nil {
			return nil, err
		}

		blockHeader, err := s.server.db.FetchBlockHeaderBySha(blockHash)
		if err != nil {
			return nil, err
		}
