...
func randIntAndElement(t *testing.T, rng *rand.Rand) (*big.Int, *Element) {
	...
	// Create and return both a big integer and a field element.
...
func TestElementSetInt(t *testing.T) {
	...
	for _, test := range tests {
		...
		if !reflect.DeepEqual(e.n, test.expected) {
			t.Errorf("%s: wrong result\ngot: %v\nwant: %v", test.name, e.n,
				test.expected)
			continue
		}
	}
...
func TestElementBytes(t *testing.T) {
	...
	for _, test := range tests {
		e := mustElementWithOverflow(test.in).Normalize()
...
func TestElementIsOne(t *testing.T) {
	tests := []struct {
		...
		name:      "2^26 (one bit in second internal limb)",
		in:        "4000000",
		normalize: false,
		expected:  false,
	}, {
		name:      "2^52 (one bit in third internal limb)",
		in:        "10000000000000",
		normalize: false,
		expected:  false,
	}, {
		name:      "2^78 (one bit in fourth internal limb)",
		in:        "40000000000000000000",
		normalize: false,
		expected:  false,
	}, {
		name:      "2^104 (one bit in fifth internal limb)",
		in:        "100000000000000000000000000",
		normalize: false,
		expected:  false,
	}, {
		name:      "2^130 (one bit in sixth internal limb)",
		in:        "400000000000000000000000000000000",
		normalize: false,
		expected:  false,
	}, {
		name:      "2^156 (one bit in seventh internal limb)",
		in:        "1000000000000000000000000000000000000000",
		normalize: false,
		expected:  false,
	}, {
		name:      "2^182 (one bit in eighth internal limb)",
		in:        "4000000000000000000000000000000000000000000000",
		normalize: false,
		expected:  false,
	}, {
		name:      "2^208 (one bit in ninth internal limb)",
		in:        "10000000000000000000000000000000000000000000000000000",
		normalize: false,
		expected:  false,
	}, {
		name:      "2^234 (one bit in tenth internal limb)",
...
// TestElementNormalize ensures that normalizing the internal limbs works as
// expected.
func TestElementNormalize(t *testing.T) {
	tests := []struct {
		...
		// Prime with internal representation such that the initial reduction
		// does not result in a carry to bit 256.
		//
	...
	for _, test := range tests {
		...
		e.n = test.raw
		e.Normalize()
		if !reflect.DeepEqual(e.n, test.normalized) {
			t.Errorf("%s: wrong normalized result\ngot: %x\nwant: %x",
				test.name, e.n, test.normalized)
			continue
		}
	}
}
...
func TestElementEquals(t *testing.T) {
	...
	for _, test := range tests {
		e := mustElementWithOverflow(test.in1).Normalize()
		e2 := mustElementWithOverflow(test.in2).Normalize()
		result := e.Equals(e2)
		if result != test.expected {
			t.Errorf("%s: wrong result -- got: %v, want: %v", test.name, result,
				test.expected)
...
func TestElementNegate(t *testing.T) {
	...
	for _, test := range tests {
		e := mustElementWithOverflow(test.in).Normalize()
		expected := mustElement(test.expected).Normalize()

		// Ensure negating another element produces the expected result.
		result := new(Element).NegateVal(e, 1).Normalize()
...
func TestElementAddInt(t *testing.T) {
	...
	for _, test := range tests {
		e := mustElementWithOverflow(test.in1).Normalize()
		expected := mustElement(test.expected).Normalize()
		result := e.AddInt(test.in2).Normalize()
		if !result.Equals(expected) {
			t.Errorf("%s: wrong result -- got: %v -- want: %v", test.name,
				result, expected)
			continue
...
func TestElementAdd(t *testing.T) {
	...
	for _, test := range tests {
		// Parse test hex.
		f1 := mustElementWithOverflow(test.in1).Normalize()
		f2 := mustElementWithOverflow(test.in2).Normalize()
		expected := mustElement(test.expected).Normalize()

		// Ensure adding the two values with the result going to another
		// variable produces the expected result.
		result := new(Element).Add2(f1, f2).Normalize()
		if !result.Equals(expected) {
			t.Errorf("%s: unexpected result\ngot: %v\nwant: %v", test.name,
				result, expected)
			continue
...
// TestElementMulByX ensures [Element.MulBy2], [Element.MulBy3],
// [Element.MulBy4], and [Element.MulBy8] produce the same results as the
// equivalent [Element.MulInt].
func TestElementMulByX(t *testing.T) {
	...
	for _, test := range tests {
		in := mustElement(test.in)
		for _, m := range mulByFuncs {
			want := new(Element).Set(in).MulInt(m.factor)
			got := m.fn(new(Element).Set(in))
			if !got.Equals(want) {
...
func TestElementMulInt(t *testing.T) {
	...
	for _, test := range tests {
		e := mustElementWithOverflow(test.in1).Normalize()
		expected := mustElement(test.expected).Normalize()
		result := e.MulInt(test.in2).Normalize()
		if !result.Equals(expected) {
			t.Errorf("%s: wrong result -- got: %v -- want: %v", test.name,
				result, expected)
			continue
...
func TestElementMul(t *testing.T) {
	...
	for _, test := range tests {
		...
		// variable produces the expected result.
		result := new(Element).Mul2(f1, f2).Normalize()
		if !result.Equals(expected) {
			t.Errorf("%s: unexpected result\ngot: %v\nwant: %v", test.name,
				result, expected)
			continue
...
// [Element.SqualVal] works as expected.
func TestElementSquare(t *testing.T) {
	...
	for _, test := range tests {
		...
		result := new(Element).SquareVal(e).Normalize()
		if !result.Equals(expected) {
			t.Errorf("%s: unexpected result\ngot: %v\nwant: %v", test.name,
				result, expected)
			continue
...
func TestElementSquareRoot(t *testing.T) {
	...
	for _, test := range tests {
		input := mustElementWithOverflow(test.in).Normalize()
		want := mustElement(test.want).Normalize()
...
func TestElementSquareRootRandom(t *testing.T) {
	...
	for i := 0; i < 100; i++ {
		...
		var elemResult Element
		elemHasSqrt := elemResult.SquareRootVal(elem)

		// Ensure they match.
		if bigIntHasSqrt != elemHasSqrt {
			t.Fatalf("mismatched square root existence\nbig int in: %x\nelem "+
				"in: %v\nbig int result: %v\nelem result %v", bigIntVal, elem,
				bigIntHasSqrt, elemHasSqrt)
		}
		if !elemHasSqrt {
			continue
		}
		bigIntResultHex := fmt.Sprintf("%064x", bigIntResult)
		elemResultHex := fmt.Sprintf("%v", elemResult)
		if bigIntResultHex != elemResultHex {
			t.Fatalf("mismatched square root\nbig int in: %x\nelem in: %v\n"+
				"big int result: %x\nelem result %v", bigIntVal, elem,
				bigIntResult, elemResult)
		}
	}
}
	...
	for i := 0; i < 100; i++ {
		...
		if bigIntResult != fValResult {
			t.Fatalf("mismatched is gt or eq prime minus order\nbig int in: "+
				"%x\nelement in: %v\nbig int result: %v\nelement result %v",
				bigIntVal, elem, bigIntResult, fValResult)
		}
	}
}
