...
// hexToFieldValWithOverflow converts the passed hex string into a [FieldVal]
...
// values.
func hexToFieldValWithOverflow(s string) *FieldVal {
	return hexToFieldValInternal(s, true)
}
...
func TestFieldBytes(t *testing.T) {
	...
	for _, test := range tests {
		f := hexToFieldValWithOverflow(test.in).Normalize()
...
func TestFieldZero(t *testing.T) {
	f := hexToFieldVal("a5a5a5a5a5a5a5a5a5a5a5a5a5a5a5a5a5a5a5a5a5a5a5a5a5a5a5a5a5a5a5a5")
	f.Zero()
...
func TestFieldIsOne(t *testing.T) {
	...
	for _, test := range tests {
		f := hexToFieldValWithOverflow(test.in)
		if test.normalize {
			f.Normalize()
		}
		result := f.IsOne()
...
func TestFieldEquals(t *testing.T) {
	...
	for _, test := range tests {
		f := hexToFieldValWithOverflow(test.in1).Normalize()
		f2 := hexToFieldValWithOverflow(test.in2).Normalize()
		result := f.Equals(f2)
		if result != test.expected {
			t.Errorf("%s: wrong result -- got: %v, want: %v", test.name, result,
				test.expected)
...
func TestFieldNegate(t *testing.T) {
	...
	for _, test := range tests {
		f := hexToFieldValWithOverflow(test.in).Normalize()
		expected := hexToFieldValWithOverflow(test.expected).Normalize()
...
func TestFieldAddInt(t *testing.T) {
	...
	for _, test := range tests {
		f := hexToFieldValWithOverflow(test.in1).Normalize()
		expected := hexToFieldVal(test.expected).Normalize()
...
func TestFieldAdd(t *testing.T) {
	...
	for _, test := range tests {
		// Parse test hex.
		f1 := hexToFieldValWithOverflow(test.in1).Normalize()
		f2 := hexToFieldValWithOverflow(test.in2).Normalize()
		expected := hexToFieldVal(test.expected).Normalize()
...
func TestFieldSquareRoot(t *testing.T) {
	...
	for _, test := range tests {
		input := hexToFieldValWithOverflow(test.in).Normalize()
		want := hexToFieldValWithOverflow(test.want).Normalize()
...
func TestFieldIsGtOrEqPrimeMinusOrder(t *testing.T) {
	...
	for _, test := range tests {
		result := hexToFieldVal(test.in).IsGtOrEqPrimeMinusOrder()
		if result != test.expected {
			t.Errorf("%s: unexpected result -- got: %v, want: %v", test.name,
				result, test.expected)
			continue
