...
// randFieldVal sets f to a field value created from a random value generated by
// the passed rng and returns f to support chaining.
func (f *FieldVal64) randFieldVal(rng *rand.Rand) *FieldVal64 {
	var buf [32]byte
	rng.Read(buf[:])
	f.SetBytes(&buf)
	return f
...
func TestField64Zero(t *testing.T) {
	...
	f.Zero()
	if !f.IsZero() {
		t.Errorf("field value is not zero - got %x", f.Bytes())
	}
}

// TestField64IsZero ensures that checking if a field is zero via IsZero works as
// expected.
func TestField64IsZero(t *testing.T) {
	f := new(FieldVal64)
	if !f.IsZero() {
		t.Errorf("new field value is not zero - got %x", f.Bytes())
	}

	f.SetInt(1)
	if f.IsZero() {
		t.Errorf("claims zero for nonzero field - got %x", f.Bytes())
	}

	f.Zero()
	if !f.IsZero() {
		t.Errorf("claims nonzero for zero field - got %x", f.Bytes())
	}
...
// TestField64IsOne ensures that checking if a field is one via IsOne works as
// expected. Unlike FieldVal, FieldVal64 is always fully reduced, so values such
// as the prime + 1 report as one.
...
func TestField64Negate(t *testing.T) {
	...
	for _, test := range tests {
		...
		result := new(FieldVal64).NegateVal(f, 1)
...
// assertField64EqualsBig fails the test when got does not equal the 32-byte
// big-endian encoding of want.
func assertField64EqualsBig(t *testing.T, want *big.Int, got *FieldVal64) {
	t.Helper()

	wantBytes := make([]byte, 32)
	want.FillBytes(wantBytes)
	if !bytes.Equal(wantBytes, got.Bytes()[:]) {
		t.Fatalf("value mismatch\nwant: %x\ngot:  %x", wantBytes, got.Bytes())
	}
}

// TestField64ReductionCarry is a deterministic regression test for the Crandall
// reduction carry-out bug. When a Mul/Square product lands in [2^256, 2^256 +
// 2^66) the second reduction fold carries out of the top limb. That carry
// represents 2^256 ≡ 0x1000003d1 (mod p); a previous implementation dropped it
// and produced a result 0x1000003d1 too small.
//
// The inputs are values of the form p-k (all strictly < p, the documented
// precondition for Mul/Square), which drive the reduction into the carry path.
// p-65537 is the smallest case originally observed to trigger the bug. These
// are intentionally fixed rather than random so the regression is always
// covered.
func TestField64ReductionCarry(t *testing.T) {
	t.Parallel()

	p := curveParams.P

	// fromBig builds a FieldVal64 from an integer that is known to be < p, so
	// SetBytes stores it without modular reduction.
	fromBig := func(v *big.Int) *FieldVal64 {
		var b [32]byte
		v.FillBytes(b[:])
		var f FieldVal64
		f.SetBytes(&b)
	...
	ks := []int64{
		1, 2, 3, 977, 978, 1953, 65536, 65537, 65538,
		0x1000003d0, 0x1000003d1, 0x1000003d2,
	}

	pMinus1Int := new(big.Int).Sub(p, big.NewInt(1))
	pMinus1 := fromBig(pMinus1Int)

	for _, k := range ks {
		aInt := new(big.Int).Sub(p, big.NewInt(k))
		a := fromBig(aInt)

		t.Run(fmt.Sprintf("square_p-%d", k), func(t *testing.T) {
			want := new(big.Int).Mul(aInt, aInt)
			want.Mod(want, p)

			var sq FieldVal64
			sq.SquareVal(a)
			assertField64EqualsBig(t, want, &sq)

			// Squaring must also agree with multiplying the value by itself.
			var prod FieldVal64
			prod.Mul2(a, a)
			if !prod.Equals(&sq) {
		...
		t.Run(fmt.Sprintf("mul_p-%d_x_p-1", k), func(t *testing.T) {
			want := new(big.Int).Mul(aInt, pMinus1Int)
			want.Mod(want, p)

			var prod FieldVal64
			prod.Mul2(a, pMinus1)
			assertField64EqualsBig(t, want, &prod)
		})
	}
}
