...
func (m *SyncManager) onInitialChainSyncDone() {
	// This is typically only called once in practice currently due to way the
	// sync process is handled, but that might change in the future with
...
func (m *SyncManager) OnTx(peer *Peer, tx *dcrutil.Tx) []*dcrutil.Tx {
	...
	// NOTE: The spec technically calls for sending an inventory message and
	// allowing the remote peer to decide whether or not they want to request
	// the transaction via a getdata message.
	//
...
func (m *SyncManager) onInitialHeaderSyncDone(hash *chainhash.Hash, height int64) {
	...
	// way the sync process is handled, but that might change in the future if
	// parallel header syncing is implemented.  Be cautious and prevent multiple
	// invocations just in case.
	//
