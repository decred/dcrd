...
// dbFetchRawGCSFilter fetches the raw version 2 GCS filter for the passed block,
// without decoding it from the db.
//
// WARNING: the returned slice is only valid for the duration of the database
// transaction and MUST be copied to a new buffer if it is needed after the
// db transaction ends.
//
...
// dbFetchGCSFilter.
func dbFetchRawGCSFilter(dbTx database.Tx, blockHash *chainhash.Hash) ([]byte, error) {
	filterBucket := dbTx.Metadata().Bucket(gcsFilterBucketName)
	serialized := filterBucket.Get(blockHash[:])
	if serialized == nil {
		return nil, nil
