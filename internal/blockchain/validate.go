...
const (
	...
	// AFSubsidySplitR2Enabled may be set to indicate that the modified subsidy
	// split agenda defined in DCP00012 should be considered as active when
	// checking a transaction so that any additional checks which depend on the
	// agenda being active are applied.
...
// IsSubsidySplitR2Enabled returns whether the flags indicate that the modified
// subsidy split agenda defined in DCP0012 is enabled.
func (flags AgendaFlags) IsSubsidySplitR2Enabled() bool {
	return flags&AFSubsidySplitR2Enabled == AFSubsidySplitR2Enabled
}
...
func (b *BlockChain) determineCheckTxFlags(prevNode *blockNode) (AgendaFlags, error) {
	...
	// Determine if the modified subsidy split round 2 agenda is active as of
	// the block being checked.
	isSubsidySplitR2Enabled, err := b.isSubsidySplitR2AgendaActive(prevNode)
	if err != nil {
		return 0, err
	}
	...
	if isSubsidySplitR2Enabled {
		checkTxFlags |= AFSubsidySplitR2Enabled
	}
...
func checkBlockSanity(block *dcrutil.Block, timeSource MedianTimeSource, flags BehaviorFlags, chainParams *chaincfg.Params) error {
	...
	for _, tx := range msgBlock.Transactions {
		err := standalone.CheckTransactionSanity(tx, maxTxSize)
		if err != nil {
			return standaloneToChainRuleError(err)
		}
	}
...
func isDCP0005Violation(network wire.CurrencyNet, header *wire.BlockHeader, blockHash *chainhash.Hash) bool {
	switch network {
	case wire.MainNet:
		// All blocks that violated DCP0005 on mainnet are version 6 and the
		// height of the last block that violated it is 430191.
		//
		// Avoid additional work for blocks that are either not that version or
		// are after the final height known to be a violation.
	...
	case wire.TestNet3:
		// All blocks that violated DCP0005 on testnet are version 7 and the
		// height of the last block that violated it is 323282.
		//
		// Return true for all blocks of that version that are before the final
		// height known to be a violation so that the DCP0005 version check is
		// not enforced until after that point.
		return header.Version == 7 && header.Height <= 323282
	}

	return false
}

// isOldBlockVersionByMajority determines if the block version specified by the
// provided header should be rejected due to a majority of the network already
// being upgraded to a newer version.
//
...
func (b *BlockChain) isOldBlockVersionByMajority(header *wire.BlockHeader, blockHash *chainhash.Hash, prevNode *blockNode) bool {
	...
	// Blocks with a version greater than or equal to the latest enforced block
	// version are most certainly not an old version.
	if header.Version >= latestBlockVersion {
		return false
	}

	// Skip the version check for blocks that are known to violate DCP0005.  See
	// isDCP0005Violation for more details.
	if isDCP0005Violation(b.chainParams.Net, header, blockHash) {
		return false
	}

	// The block version is considered old once the majority of the network has
	// upgraded to a more recent version.
	rejectNumRequired := b.chainParams.BlockRejectNumRequired
...
func (b *BlockChain) checkBlockHeaderPositional(header *wire.BlockHeader, prevNode *blockNode, flags BehaviorFlags) error {
	...
	if !fastAdd {
		...
		// upgraded.
		if b.isOldBlockVersionByMajority(header, &blockHash, prevNode) {
...
func checkVoteInputs(subsidyCache *standalone.SubsidyCache, tx *dcrutil.Tx,
	...
	subsidySplitVariant standalone.SubsidySplitVariant) error {
	...
	voteSubsidy := subsidyCache.CalcStakeVoteSubsidyV3(int64(heightVotingOn),
		subsidySplitVariant)
...
func checkStakeBaseAmounts(subsidyCache *standalone.SubsidyCache, height int64,
	txs []*dcrutil.Tx, view *UtxoViewpoint,
	subsidySplitVariant standalone.SubsidySplitVariant) error {
	...
	for _, tx := range txs {
		...
		if stake.IsSSGen(msgTx) {
			...
			calcSubsidy := subsidyCache.CalcStakeVoteSubsidyV3(height-1,
				subsidySplitVariant)
...
func (b *BlockChain) checkTransactionsAndConnect(inputFees dcrutil.Amount,
	...
	subsidySplitVariant standalone.SubsidySplitVariant) (dcrutil.Amount, error) {
	...
	if !stakeTree { // TxTreeRegular
		...
		if node.height == 1 {
			...
			subsidyWork := b.subsidyCache.CalcWorkSubsidyV3(node.height,
		...
		if (coinbaseIn.ValueIn != subsidyWithoutFees) &&
			...
			errStr := fmt.Sprintf("bad coinbase subsidy in input;"+
				...
				subsidyWithoutFees)
			return 0, ruleError(ErrBadCoinbaseAmountIn, errStr)
		}
		...
		if node.height > 1 && isTreasuryEnabled {
			if len(txs) == 0 {
				...
				str := fmt.Sprintf("empty tx tree stake, "+
					...
					node.height)
				return 0, ruleError(ErrNoStakeTx, str)
			}
			...
			if treasurybaseIn.ValueIn != subsidyTax {
				errStr := fmt.Sprintf("bad treasurybase "+
					...
					subsidyTax)
				return 0, ruleError(ErrBadTreasurybaseAmountIn, errStr)
			}
		}

		if len(txs) == 0 &&
			node.height < b.chainParams.StakeValidationHeight {
			return dcrutil.Amount(totalFees), nil
		}
		if len(txs) == 0 &&
			node.height >= b.chainParams.StakeValidationHeight {
		...
		if totalAtomOutStake > expAtomOut {
			str := fmt.Sprintf("stakebase transactions for block pays %v "+
				...
				expAtomOut)
			return 0, ruleError(ErrBadStakebaseValue, str)
		}
	}
...
func (b *BlockChain) checkConnectBlock(node *blockNode, block, parent *dcrutil.Block, view *UtxoViewpoint, stxos *[]spentTxOut, hdrCommitments *headerCommitmentData) error {
	...
	stakeTreeFees, err := b.checkTransactionsAndConnect(0, node,
		block.STransactions(), view, stxos, stakeTreeTrue, subsidySplitVariant)
	if err != nil {
	...
	_, err = b.checkTransactionsAndConnect(stakeTreeFees, node,
