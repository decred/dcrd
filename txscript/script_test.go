...
func TestRemoveOpcodeByData(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name: "stakesubmission simple case length 6",
			before: append(append(append([]byte{OP_SSTX, OP_DUP, OP_HASH160, OP_DATA_20},
				bytes.Repeat([]byte{0}, 16)...), []byte{1, 2, 3, 4}...), []byte{OP_EQUALVERIFY, OP_CHECKSIG}...),
			remove: []byte{1, 2, 3, 4},
			after:  []byte{OP_SSTX, OP_DUP, OP_HASH160, OP_EQUALVERIFY, OP_CHECKSIG},
		},
		{
			name: "stakesubmission simple case length 6 (miss)",
			before: append(append(append([]byte{OP_SSTX, OP_DUP, OP_HASH160, OP_DATA_20},
				bytes.Repeat([]byte{0}, 16)...), []byte{1, 2, 3, 4}...), []byte{OP_EQUALVERIFY, OP_CHECKSIG}...),
			remove: []byte{1, 2, 3, 4, 5},
			after: append(append(append([]byte{OP_SSTX, OP_DUP, OP_HASH160, OP_DATA_20},
				bytes.Repeat([]byte{0}, 16)...), []byte{1, 2, 3, 4}...), []byte{OP_EQUALVERIFY, OP_CHECKSIG}...),
		},
		{
			name: "stakesubmission simple case length 4",
			before: append(append(append([]byte{OP_SSTX, OP_HASH160, OP_DATA_20},
				bytes.Repeat([]byte{0}, 16)...), []byte{1, 2, 3, 4}...), []byte{OP_EQUAL}...),
			remove: []byte{1, 2, 3, 4},
			after:  []byte{OP_SSTX, OP_HASH160, OP_EQUAL},
		},
		{
			name: "stakesubmission simple case length 4 (miss)",
			before: append(append(append([]byte{OP_SSTX, OP_HASH160, OP_DATA_20},
				bytes.Repeat([]byte{0}, 16)...), []byte{1, 2, 3, 4}...), []byte{OP_EQUAL}...),
			remove: []byte{1, 2, 3, 4, 5},
			after: append(append(append([]byte{OP_SSTX, OP_HASH160, OP_DATA_20},
				bytes.Repeat([]byte{0}, 16)...), []byte{1, 2, 3, 4}...), []byte{OP_EQUAL}...),
		},
		{
			name: "stakegen simple case length 6",
			before: append(append(append([]byte{OP_SSGEN, OP_DUP, OP_HASH160, OP_DATA_20},
				bytes.Repeat([]byte{0}, 16)...), []byte{1, 2, 3, 4}...), []byte{OP_EQUALVERIFY, OP_CHECKSIG}...),
			remove: []byte{1, 2, 3, 4},
			after:  []byte{OP_SSGEN, OP_DUP, OP_HASH160, OP_EQUALVERIFY, OP_CHECKSIG},
		},
		{
			name: "stakegen simple case length 6 (miss)",
			before: append(append(append([]byte{OP_SSGEN, OP_DUP, OP_HASH160, OP_DATA_20},
				bytes.Repeat([]byte{0}, 16)...), []byte{1, 2, 3, 4}...), []byte{OP_EQUALVERIFY, OP_CHECKSIG}...),
			remove: []byte{1, 2, 3, 4, 5},
			after: append(append(append([]byte{OP_SSGEN, OP_DUP, OP_HASH160, OP_DATA_20},
				bytes.Repeat([]byte{0}, 16)...), []byte{1, 2, 3, 4}...), []byte{OP_EQUALVERIFY, OP_CHECKSIG}...),
		},
		{
			name: "stakegen simple case length 4",
			before: append(append(append([]byte{OP_SSGEN, OP_HASH160, OP_DATA_20},
				bytes.Repeat([]byte{0}, 16)...), []byte{1, 2, 3, 4}...), []byte{OP_EQUAL}...),
			remove: []byte{1, 2, 3, 4},
			after:  []byte{OP_SSGEN, OP_HASH160, OP_EQUAL},
		},
		{
			name: "stakegen simple case length 4 (miss)",
			before: append(append(append([]byte{OP_SSGEN, OP_HASH160, OP_DATA_20},
				bytes.Repeat([]byte{0}, 16)...), []byte{1, 2, 3, 4}...), []byte{OP_EQUAL}...),
			remove: []byte{1, 2, 3, 4, 5},
			after: append(append(append([]byte{OP_SSGEN, OP_HASH160, OP_DATA_20},
				bytes.Repeat([]byte{0}, 16)...), []byte{1, 2, 3, 4}...), []byte{OP_EQUAL}...),
		},
		{
			name: "stakerevoke simple case length 6",
			before: append(append(append([]byte{OP_SSRTX, OP_DUP, OP_HASH160, OP_DATA_20},
				bytes.Repeat([]byte{0}, 16)...), []byte{1, 2, 3, 4}...), []byte{OP_EQUALVERIFY, OP_CHECKSIG}...),
			remove: []byte{1, 2, 3, 4},
			after:  []byte{OP_SSRTX, OP_DUP, OP_HASH160, OP_EQUALVERIFY, OP_CHECKSIG},
		},
		{
			name: "stakerevoke simple case length 6 (miss)",
			before: append(append([]byte{OP_SSRTX, OP_DUP, OP_HASH160, OP_DATA_20},
				bytes.Repeat([]byte{0}, 20)...), []byte{OP_EQUALVERIFY, OP_CHECKSIG}...),
			remove: bytes.Repeat([]byte{0}, 21),
			after: append(append([]byte{OP_SSRTX, OP_DUP, OP_HASH160, OP_DATA_20},
				bytes.Repeat([]byte{0}, 20)...), []byte{OP_EQUALVERIFY, OP_CHECKSIG}...),
		},
		{
			name: "stakerevoke simple case length 4",
			before: append(append(append([]byte{OP_SSRTX, OP_HASH160, OP_DATA_20},
				bytes.Repeat([]byte{0}, 16)...), []byte{1, 2, 3, 4}...), []byte{OP_EQUAL}...),
			remove: []byte{1, 2, 3, 4},
			after:  []byte{OP_SSRTX, OP_HASH160, OP_EQUAL},
		},
		{
			name: "stakerevoke simple case length 4 (miss)",
			before: append(append(append([]byte{OP_SSRTX, OP_HASH160, OP_DATA_20},
				bytes.Repeat([]byte{0}, 16)...), []byte{1, 2, 3, 4}...), []byte{OP_EQUAL}...),
			remove: []byte{1, 2, 3, 4, 5},
			after: append(append(append([]byte{OP_SSRTX, OP_HASH160, OP_DATA_20},
				bytes.Repeat([]byte{0}, 16)...), []byte{1, 2, 3, 4}...), []byte{OP_EQUAL}...),
		},
		{
