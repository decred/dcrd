...
func handleTooFewVoters(subsidyCache *blockchain.SubsidyCache, nextHeight int64, miningAddress dcrutil.Address, bm *blockManager) (*BlockTemplate, error) {
	...
	if curTemplate != nil {
		if !best.PrevHash.IsEqual(&curTemplate.Block.Header.PrevBlock) {
...
// handleTooFewVotersV2 handles the situation in which there are too few voters on
// of the blockchain. If there are too few voters and a cached parent template to
// work off of is present, it will return a copy of that template to pass to the
// miner.
// Safe for concurrent access.
func handleTooFewVotersV2(subsidyCache *blockchain.SubsidyCache, nextHeight int64, miningAddress dcrutil.Address, bm *blockManager, currentTmpl *BlockTemplate) (*BlockTemplate, error) {
	timeSource := bm.server.timeSource
	stakeValidationHeight := bm.server.chainParams.StakeValidationHeight

	// Check to see if we've fallen off the chain, for example if a
	// reorganization had recently occurred. If this is the case,
	// nuke the current template.
	best := bm.chain.BestSnapshot()
	if currentTmpl != nil {
		if !best.PrevHash.IsEqual(&currentTmpl.Block.Header.PrevBlock) {
			minrLog.Debugf("template is no longer current, resetting.")
			currentTmpl = nil
		}
	}
	...
	if nextHeight >= stakeValidationHeight {
		if bm.AggressiveMining {
			if currentTmpl != nil {
				cptCopy := deepCopyBlockTemplate(currentTmpl)
				...
				// Update extranonce of the original template too, so
				// we keep getting unique numbers.
				err = UpdateExtraNonce(currentTmpl.Block, currentTmpl.Height, en)
				if err != nil {
					return nil, err
				}
...
mempoolLoop:
	...
	minrLog.Debugf("Created new block template (%d height, %d transactions, "+
		"%d stake transactions, %d in fees, %d signature operations, "+
		"%d bytes, target difficulty %064x, stake difficulty %v)",
		msgBlock.Header.Height, len(msgBlock.Transactions),
		len(msgBlock.STransactions), totalFees, blockSigOps, blockSize,
...
// NewBlockTemplateV2 returns a new block template that is ready to be solved
// using the transactions from the passed transaction source pool and a coinbase
// that either pays to the passed address if it is not nil, or a coinbase that
// is redeemable by anyone if the passed address is nil.  Refer to
// NewBlockTemplate for more detail.
//
// This function returns nil, nil if there are not enough voters on any of
//  the current top blocks to create a new block template.
func (g *BlkTmplGenerator) NewBlockTemplateV2(payToAddress dcrutil.Address, currentTmpl *BlockTemplate) (*BlockTemplate, error) {
	subsidyCache := g.chain.FetchSubsidyCache()
	...
	if nextBlockHeight >= stakeValidationHeight {
		...
		if len(eligibleParents) == 0 {
			minrLog.Debugf("Too few voters found on any HEAD block, " +
				"recycling a parent block to mine on")
			return handleTooFewVotersV2(subsidyCache, nextBlockHeight,
				payToAddress, g.blockManager, currentTmpl)
		}
...
mempoolLoop:
	...
	if nextBlockHeight >= stakeValidationHeight &&
		...
		minrLog.Warnf("incongruent number of voters in mempool " +
			"vs mempool.voters; not enough voters found")
		return handleTooFewVotersV2(subsidyCache, nextBlockHeight, payToAddress,
			g.blockManager, currentTmpl)
	}
...
func (g *BgBlkTmplGenerator) regenTemplate(ctx context.Context) {
	...
	template, err := g.tg.NewBlockTemplateV2(payToAddr, tmpl)
