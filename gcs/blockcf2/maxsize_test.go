...
func TestMaxSize(t *testing.T) {
	...
	// entry on a filter may be unique (i.e. no two scripts will generate
	// the same internal value after siphash and modulo reductions).
	...
	// minTxSize is the minimum transaction size with one input (and no outputs).
	minTxSize := uint32(12 + 57 + 3 + 2)

	// txOutSize is the size of an output, plus one byte for the pkscript
	// length encoded as a varint.
	txOutSize := uint32(10 + 1)

	// p2shSize is the size of a P2SH script (the smallest standard script
	// that may be added without limits to a transaction).
	p2shSize := uint32(23)
	...
	tests := []struct {
		...
		// This test asserts the size of a filter when the source data
		// for the filter is a tx that has as many OP_RETURN outputs
		// as necessary to fill a block.
		name: "filter from tx filled with OP_RETURN outputs",
		n:    wire.MaxBlockPayload / (11 + entrySizeForSize(wire.MaxBlockPayload)),
		want: 251581,
	}, {
		// This test asserts the size of a filter when the source data
		...
		n:    wire.MaxBlockPayload / (minTxSize + (txOutSize+entrySizeForSize(wire.MaxBlockPayload))*4) * 4,
		want: 108370,
	}, {
		// This test asserts the size of a filter when the source data
...
func TestWorstMaxSize(t *testing.T) {
	...
	// will generate a value (i.e. no removed duplicates) and the last script
	// will generate the max possible quotient.
