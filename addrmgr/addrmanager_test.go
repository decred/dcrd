...
func TestAttempt(t *testing.T) {
	...
	unknownNetAddress := NewNetAddressIPPort(unknownIP, 1234, sfNodeNetwork)
...
func TestNeedMoreAddresses(t *testing.T) {
	...
	if !b {
		t.Error("expected the address manager to need more addresses")
	}
	addrs := make([]*NetAddress, addrsToAdd)

	for i := 0; i < addrsToAdd; i++ {
		s := fmt.Sprintf("%d.%d.173.147", i/128+60, i%128+60)
		addrs[i] = NewNetAddressIPPort(net.ParseIP(s), 8333, sfNodeNetwork)
	}
	...
	b = n.NeedMoreAddresses()
	if b {
		t.Error("expected address manager to not need more addresses")
	}
}
...
func TestGood(t *testing.T) {
	...
	srcAddr := NewNetAddressIPPort(net.ParseIP("173.144.173.111"), 8333, sfNodeNetwork)
	...
	if n.addrTried[0][0].na.Key() != addrBKey {
		t.Errorf("expected address %s to exist in tried bucket", addrBKey)
	}
	if _, exists := n.addrNew[0][addrAKey]; !exists {
		t.Errorf("expected address %s to exist in the new bucket after being "+
...
func TestGetAddress(t *testing.T) {
	...
	if rv := n.GetAddress(); rv != nil {
		t.Errorf("GetAddress failed - got: %v, want: %v", rv, nil)
	}
...
func TestGetBestLocalAddress(t *testing.T) {
	newAddressFromIP := func(ip net.IP) *NetAddress {
		const port = 0
		return NewNetAddressIPPort(ip, port, sfNodeNetwork)
	}
...
func TestValidatePeerNa(t *testing.T) {
	...
	for _, test := range tests {
		...
		localNa := NewNetAddressIPPort(localIP, 8333, sfNodeNetwork)
		remoteNa := NewNetAddressIPPort(remoteIP, 8333, sfNodeNetwork)
