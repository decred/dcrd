...
func (bc *bucketStats) increment(addrType NetAddressType) {
	switch addrType {
	case IPv4Address:
		bc.numIPv4++
	case IPv6Address:
...
func (bc *bucketStats) decrement(addrType NetAddressType) {
	switch addrType {
	case IPv4Address:
		bc.numIPv4--
	case IPv6Address:
...
func (bc *bucketStats) total(filter addrTypeFilter) int {
	sum := 0
	if filter.wantIPv4 {
		sum += int(bc.numIPv4)
	}
...
func (bc *bucketStats) matches(filter addrTypeFilter) bool {
	return (filter.wantIPv4 && bc.numIPv4 > 0) ||
		(filter.wantIPv6 && bc.numIPv6 > 0) ||
		(filter.wantTORv3 && bc.numTORv3 > 0)
}
