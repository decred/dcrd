...
func TestKey(t *testing.T) {
	...
	for _, test := range tests {
		netAddr := NewNetAddressIPPort(net.ParseIP(test.ip), test.port, sfNodeNetwork)
		key := netAddr.Key()
		if key != test.want {
...
func TestClone(t *testing.T) {
	const port = 0
	netAddr := NewNetAddressIPPort(net.ParseIP("1.2.3.4"), port, sfNodeNetwork)
	...
	if !reflect.DeepEqual(netAddr, netAddrClone) {
		t.Errorf("unxpected clone result -- got %v, want %v",
...
func TestAddService(t *testing.T) {
	const port = 0
	netAddr := NewNetAddressIPPort(net.ParseIP("1.2.3.4"), port, ServiceFlag(0))
