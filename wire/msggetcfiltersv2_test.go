...
// baseMsgGetCFiltersV2 returns a MsgGetCFiltersV2 struct populated with mock
...
func baseMsgGetCFiltersV2(t *testing.T) *MsgGetCFiltersV2 {
	t.Helper()
	...
	return NewMsgGetCFiltersV2(startHash, endHash)
}

// TestGetCFiltersV2 tests the MsgGetCFiltersV2 API against the latest protocol
// version.
func TestGetCFiltersV2(t *testing.T) {
	...
	wantCmd := "getcfsv2"
	msg := baseMsgGetCFiltersV2(t)
	if cmd := msg.Command(); cmd != wantCmd {
		t.Errorf("NewMsgGetCFiltersV2: wrong command - got %v want %v", cmd,
			wantCmd)
	}
...
// TestGetCFiltersV2PreviousProtocol tests the MsgGetCFiltersV2 API against the
...
// TestGetCFiltersV2CrossProtocol tests the MsgGetCFiltersV2 API when encoding
...
func TestGetCFiltersV2CrossProtocol(t *testing.T) {
	...
	if err != nil {
		t.Errorf("encode of MsgGetCFiltersV2 failed %v err <%v>", msg, err)
	}
	...
	if err != nil {
		t.Errorf("decode of MsgGetCFiltersV2 failed [%v] err <%v>", buf, err)
	}
}

// TestGetCFiltersV2Wire tests the MsgGetCFiltersV2 wire encode and decode for
// various commitment names and protocol versions.
func TestGetCFiltersV2Wire(t *testing.T) {
	// MsgGetCFiltersV2 message with mock block hashes.
	msgGetCFiltersV2 := baseMsgGetCFiltersV2(t)
	msgGetCFiltersV2Encoded := []byte{
	...
	tests := []struct {
		in   *MsgGetCFiltersV2 // Message to encode
		out  *MsgGetCFiltersV2 // Expected decoded message
...
// decode of MsgGetCFiltersV2 to confirm error paths work correctly.
func TestGetCFiltersV2WireErrors(t *testing.T) {
	pver := ProtocolVersion

	// MsgGetCFilterV2 message with mock block hash.
	baseGetCFiltersV2 := baseMsgGetCFiltersV2(t)
	...
	tests := []struct {
		in       *MsgGetCFiltersV2 // Value to encode
		buf      []byte            // Wire encoding
		pver     uint32            // Protocol version for wire encoding
