...
var (
	...
	ipv6Address  = newNetAddressV2(IPv6Address, ipv6IpBytes, 8333)
	...
	serializedIPv6NetAddressBytes = []byte{
		...
		0x01, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, // Services
		0x02,                                           // Type (IPv6)
		0x10,                                           // Varint IP length
		0x26, 0x20, 0x01, 0x00, 0x00, 0x00, 0x00, 0x00, // IP (upper)
		0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x01, // IP (lower)
		0x8d, 0x20, // Port 8333 (little-endian)
	}
	serializedUnknownNetAddressBytes = bytes.Join([][]byte{
		{
			0x29, 0xab, 0x5f, 0x49, 0x00, 0x00, 0x00, 0x00, // Timestamp
			0x01, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, // Services
			0x00,             // Type (Unknown)
			0xfd, 0x00, 0x02, // Varint IP length (512)
		},
		make([]byte, MaxAddressSize), // IP
		{0x8d, 0x20},                 // Port 8333 (little-endian)
	}, []byte{})
...
// TestBtcDecode verifies decode behavior for various error conditions and
// forward compatibility with unknown address types.
func TestBtcDecode(t *testing.T) {
	pver := ProtocolVersion
	...
	tests := []struct {
		...
		name: "message with zero length address",
		wireBytes: []byte{
			0x01,                                           // Varint address count
			0x29, 0xab, 0x5f, 0x49, 0x00, 0x00, 0x00, 0x00, // Timestamp
			0x01, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, // Services
			0x02,       // Type (UnknownAddressType)
			0x00,       // Varint IP length
			0x8d, 0x20, // Port 8333 (little-endian)
		},
		wantAddrs: nil,
		...
		name: "message with oversized address",
		wireBytes: bytes.Join([][]byte{
			{
				...
				0x01, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, // Services
				0x01,             // Type (IPv4)
				0xfd, 0x01, 0x02, // Varint IP length = 513
			},
			make([]byte, 513), // 513 byte IP
			{0x8d, 0x20},      // Port 8333 (little-endian)
		}, []byte{}),
		wantAddrs: nil,
		wantErr:   ErrInvalidMsg,
	}, {
		name: "max size message with only unknown types",
		wireBytes: func() []byte {
			message := [][]byte{{0xfd, 0xe8, 0x03}} // Varint - MaxAddrPerV2Msg
			for i := 0; i < MaxAddrPerV2Msg; i++ {
				message = append(message, serializedUnknownNetAddressBytes)
			}
		...
		name: "multiple valid serialized addresses",
		wireBytes: bytes.Join([][]byte{
			{0x03},
			serializedIPv4NetAddressBytes,
		...
		wantAddrs: []NetAddressV2{ipv4Address, ipv6Address},
		wantErr:   nil,
	}}
	...
	for _, test := range tests {
		...
		ioLimit := int(msg.MaxPayloadLength(pver))
		bytesRead := rbuf.Len()
		if bytesRead > ioLimit {
			t.Errorf("%q: read %d bytes, which exceeds max payload length %d",
				test.name, bytesRead, ioLimit)
			continue
		}
...
// TestBtcEncode performs negative tests against wire encoding
// of MsgAddrV2 to confirm error paths work correctly.
func TestBtcEncode(t *testing.T) {
	pver := ProtocolVersion
	na := ipv4Address
	...
	tests := []struct {
		...
		wantErr error          // Expected encode error
	}{{
		name:    "message with no addresses",
		pver:    pver,
		addrs:   nil,
		...
		name: "message with maximum number of addresses",
		pver: pver,
		addrs: func() []NetAddressV2 {
			var addrs []NetAddressV2
			for i := 0; i < MaxAddrPerV2Msg; i++ {
