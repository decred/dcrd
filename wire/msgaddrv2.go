...
// MaxAddressSize is the maximum size in bytes for a network address.
const MaxAddressSize = 512
...
func readNetAddressV2(op string, r io.Reader, pver uint32, na *NetAddressV2) error {
	...
	// Read the address length for forward compatibility with future address types.
	addrLen, err := ReadVarInt(r, pver)
	if err != nil {
		return err
	}

	// Sanity check claimed address length.
	if addrLen == 0 || addrLen > MaxAddressSize {
		msg := fmt.Sprintf("invalid address length: %d", addrLen)
	...
	default:
		...
		// the address when constructing the message payload.
		// This allows the addrv2 message to accommodate unknown
		// address types without requiring protocol version
		// upgrades.
		_, err := io.CopyN(io.Discard, r, int64(addrLen))
		if err != nil {
			return err
		}
	}
...
func writeNetAddressV2(op string, w io.Writer, pver uint32, na NetAddressV2) error {
	...
	default:
		msg := fmt.Sprintf("cannot encode unknown network address type %v", na.Type)
...
func (msg *MsgAddrV2) BtcDecode(r io.Reader, pver uint32) error {
	...
	// Pre-allocate slice with capacity for the claimed address count.
	validAddrs := make([]NetAddressV2, 0, count)

	for i := uint64(0); i < count; i++ {
		var na NetAddressV2
		err := readNetAddressV2(op, r, pver, &na)
		if err != nil {
			return err
		}

		// Only include addresses with non-nil IP. Known address types have
		// their IP field populated. Unknown types have nil IP after being
		// read and discarded, which signals they should be filtered out.
		if na.IP != nil {
			validAddrs = append(validAddrs, na)
		}
	}

	msg.AddrList = validAddrs
	return nil
}
...
func maxNetAddressPayloadV2() uint32 {
	const (
		...
		servicesSize    = 8
		addressTypeSize = 1
		maxLengthSize   = 3 // varint size for MaxAddressSize
		portSize        = 2
	)
	return timestampSize + servicesSize + addressTypeSize +
		maxLengthSize + MaxAddressSize + portSize
}
