...
// provided values unless banning is disabled.
//
// A warning is logged including the reason provided when the resulting score
// exceeds half of the ban threshold and, unless the peer is whitelisted, it
// will be banned and disconnected when the score exceeds the ban threshold.
func (sp *serverPeer) addBanScore(persistent, transient uint32, reason string) bool {
	...
	warnThreshold := cfg.BanThreshold / 2
	score := sp.banScore.Increase(persistent, transient)
	if score > warnThreshold {
		peerStr := "peer"
		if sp.isWhitelisted {
			peerStr = "whitelisted peer"
		}
		srvrLog.Warnf("Misbehaving %s %s: %s -- ban score increased to %d",
			peerStr, sp, reason, score)

		// Ban the peer once the threshold is exceeded unless it is whitelisted.
		if score > cfg.BanThreshold && !sp.isWhitelisted {
...
func (sp *serverPeer) onMixMessage(msg mixing.Message) {
	...
	// clients.
	//
	// The accepted slice is intentionally used when non-empty before checking
	// the error because accepting a message can cause a cascade of other
	// messages that were previously orphans to be considered too and it is
	// possible that one or more of those previous orphans are rejected which
	// results in a mix of accepted messages and rejected messages.  The
	// accepted slice contains the ones that were successfully accepted and thus
	// they still need to be announced.
	//
...
// possibly be reachable via one of them.
//
// This function MUST be called with the embedded mutex locked (reads).
...
func (s *server) considerReportedAddrOutbound(from *serverPeer, addr *wire.NetAddress) {
	...
	if candidate.score < math.MaxUint32 {
		candidate.score++
	}
	...
	case blockchain.NTBlockAccepted:
		...
		case s.rpcServer != nil &&
			...
			if s.mixObserver.MisbehavingBlock(block.MsgBlock()) {
				syncLog.Debugf("Refusing to vote on block spending " +
					"misbehaving mix inputs")
				break
			}
...
func newServer(ctx context.Context, profiler *profileServer,
	...
	s.chain, err = blockchain.New(ctx,
		&blockchain.Config{
			...
			AllowOldForks:   cfg.AllowOldForks,
