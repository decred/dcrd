...
func newServer(listenAddrs []string, db database.DB, chainParams *chaincfg.Params, dataDir string, interrupt <-chan struct{}) (*server, error) {
	...
	if !cfg.DisableRPC {
		s.rpcServer, err = newRPCServer(&rpcserverConfig{
			...
			SyncMgr: &rpcSyncMgr{
				IsCurrentImpl:          s.blockManager.IsCurrent,
				SubmitBlockImpl:        s.blockManager.ProcessBlock,
				SyncPeerIDImpl:         s.blockManager.SyncPeerID,
				LocateBlocksImpl:       s.chain.LocateBlocks,
				ExistsAddrIndexImpl:    s.existsAddrIndex,
				CFIndexImpl:            s.cfIndex,
				TipGenerationImpl:      s.blockManager.TipGeneration,
				SyncHeightImpl:         s.blockManager.SyncHeight,
				ProcessTransactionImpl: s.blockManager.ProcessTransaction,
			},
			PeerNotifier: &s,
			FeeEstimator: s.feeEstimator,
			TimeSource:   s.timeSource,
