...
func newNode(blockVersion int32, height int64, currentNode *blockNode) *blockNode {
	// Make up a header.
...
func TestCalcStakeVersionCorners(t *testing.T) {
	...
	for i := int64(0); i < runCount; i++ {
		node := newNode(3, height+i, currentNode)
		...
		sv, _ := bc.calcStakeVersionByNode(currentNode)
...
func TestIsStakeMajorityVersion(t *testing.T) {
	...
	for _, test := range tests {
		...
		version, _ := bc.calcStakeVersionByNode(currentNode)
