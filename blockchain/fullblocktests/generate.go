...
func Generate(includeLargeReorg bool) (tests [][]TestInstance, err error) {
	...
	rejectBlock := func(blockName string, block *wire.MsgBlock, errKind blockchain.ErrorKind) TestInstance {
		return RejectedBlock{blockName, block, errKind}
	}
	...
	rejected := func(errKind blockchain.ErrorKind) {
		tests = append(tests, []TestInstance{
			rejectBlock(g.TipName(), g.Tip(), errKind),
		})
	}
