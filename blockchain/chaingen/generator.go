...
const (
	...
	// TSDecentralized specifies the decentralized treasury semantics introduced
	// by DCP0006.
	TSDecentralized
)
...
func (g *Generator) UseTreasurySemantics(semantics TreasurySemantics, options ...string) {
	...
	case TSDecentralized:
		if len(options) != 0 {
			panic("no options may be specified for the decentralized " +
				"treasury semantics")
		}
		g.treasurySemantics = semantics
		return
...
func (g *Generator) calcCoinbaseDevSubsidy(fullSubsidy dcrutil.Amount, blockHeight uint32, numVotes uint16) dcrutil.Amount {
	if g.treasurySemantics == TSDecentralized && blockHeight > 1 {
		return 0
	}
...
func (g *Generator) addCoinbaseTxOutputs(tx *wire.MsgTx, blockHeight uint32, devSubsidy, powSubsidy dcrutil.Amount) {
	...
	//
	// It will either be the first output when the decentralized treasury
	// semantics are in effect or the second when the original treasury
...
//
// For the original treasury semantics (TSOriginal) the outputs are:
...
//
// For the decentralized treasury semantics (TSDecentralized) the outputs are:
...
//
//   - When the original treasury semantics ([TSOriginal]) are active, one that
//     pays the required 10% subsidy to the dev org
...
//
// 4. When the decentralized treasury semantics are active ([TSDecentralized]),
// a treasurybase with the following outputs:
//
...
func (g *Generator) NextBlock(blockName string, spend *SpendableOut, ticketSpends []SpendableOut, mungers ...func(*wire.MsgBlock)) *wire.MsgBlock {
	...
	if g.treasurySemantics == TSDecentralized && nextHeight > 1 {
...
func (g Generator) countSigOps(script []byte, height uint32) int {
	isTreasuryEnabled := g.treasurySemantics == TSDecentralized && height > 1
